...
const AdminDataPlans = () => {
  ...
  const fetchPlans = async () => {
    try {
      const data = await api("/api/admin/data-plans");
      setPlans(data);
    } catch (err) {
  ...
  const handleCreate = async () => {
    ...
    setFormLoading(true);
    try {
      await api("/api/admin/data-plans", {
        method: "POST",
        body: JSON.stringify({
          ...form,
  ...
  const handleToggle = async (plan) => {
    try {
      await api(`/api/admin/data-plans/${plan._id}`, {
        method: "PATCH",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api(`/api/admin/data-plans/${id}`, { method: "DELETE" });
    ...
    <div className="p-6">
      ...
      {showForm && (
        <div className="bg-white border rounded-xl p-4 mb-4 space-y-3">
          <h2 className="font-semibold">New Data Plan</h2>
          {formError && (
          ...
          {[
            { key: "planCode", placeholder: "Plan Code (e.g SME1GB)" },
            { key: "planName", placeholder: "Plan Name (e.g MTN SME 1GB)" },
            { key: "size", placeholder: "Size (e.g 1GB)" },
